...
const expectedResult = [
  ...
  function fantasyOrScienceFictionAuthors() {
      return books.filter((item) => item.genre === 'Fantasia' || item.genre === 'Ficção Científica').map((item2) => item2.author.name).sort();
  }
 ...
 const expectedResult = [
  ...
  function oldBooks() {
      return books.filter((item) => item.releaseYear < 1961).map((item2) => item2.name);
  }
